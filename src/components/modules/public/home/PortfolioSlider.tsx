...
const PortfolioSlider = ({ portfolios }: PortfolioSliderProps) => {
  ...
  return (
    <section className="py-24 relative bg-background text-foreground transition-colors duration-500 overflow-hidden">
      ...
      <Container>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div className="max-w-2xl">
            ...
            >
              A curated selection of our most impactful work. Each project represents our 
              commitment to innovation and digital excellence.
            </motion.p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <button className="portfolio-prev w-12 h-12 rounded-2xl glass-premium flex items-center justify-center hover:bg-primary hover:text-white transition-all cursor-pointer border border-border/50 shadow-xl group">
                <ChevronLeft className="w-6 h-6 group-hover:-translate-x-1 transition-transform" />
              </button>
              <button className="portfolio-next w-12 h-12 rounded-2xl glass-premium flex items-center justify-center hover:bg-primary hover:text-white transition-all cursor-pointer border border-border/50 shadow-xl group">
            ...
            >
              <Link
                href="/portfolio"
                className="group flex items-center gap-2 glass-premium px-8 py-4 rounded-2xl hover:bg-primary hover:text-white transition-all font-bold tracking-wide shadow-xl border border-border/50"
              >
                View Gallery
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-8 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                      <div className="flex flex-wrap gap-2 mb-4 opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-100">
                        {project.technologies.slice(0, 2).map((tech) => (
                          <span key={tech} className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-primary/20 text-primary backdrop-blur-md border border-primary/20">
                            {tech}
                          </span>
                        ))}
                      </div>

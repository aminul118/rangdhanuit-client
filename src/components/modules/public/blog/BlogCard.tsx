...
const BlogCard = ({ blog }: BlogCardProps) => {
  return (
    <motion.div
      ...
      {...HOVER_LIFT}
      className="relative overflow-hidden rounded-[2rem] md:rounded-[2.5rem] glass border-border/50 backdrop-blur-xl h-full flex flex-col transition-all duration-500 hover:border-indigo-500/30 hover:shadow-[0_20px_50px_-20px_rgba(99,102,241,0.2)]"
    >
      ...
      <div className="p-6 md:p-8 flex flex-col grow">
        ...
        <div className="mt-auto pt-6 border-t border-border/50">
          <Link
            href={`/blog/${blog.slug}`}
            className="inline-flex items-center gap-2 text-indigo-400 font-black text-xs uppercase tracking-widest group/link hover:text-indigo-500 transition-all"
          >
            Read Full Article
            <ArrowRight
              size={18}

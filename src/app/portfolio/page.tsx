  ...
  return (
    <div className="flex flex-col gap-24 pb-24">
      ...
      <section className="pt-20 pb-12 text-center container mx-auto px-6">
        <h1 className="text-4xl md:text-6xl font-bold mb-8">Our <span className="gradient-text">Portfolio</span></h1>
      ...
      <section className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {projects.map((project) => (
            <div key={project.title} className="glass group overflow-hidden rounded-[40px] flex flex-col">
              <div className="h-64 bg-white/5 relative flex items-center justify-center overflow-hidden">
                <project.icon className="w-24 h-24 text-muted-foreground/20 group-hover:scale-110 transition-transform duration-500" />
                <div className="absolute top-8 left-8">
                  <span className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider ${project.color}`}>
                    {project.category}
                  </span>
                </div>
              </div>
